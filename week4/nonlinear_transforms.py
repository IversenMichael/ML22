...
class PerceptronClassifier():
    ...
    def fit(self, X, y, maxiter=1 << 16, w=None):
        ...
        for i in range(maxiter):
            idx_incorrect = np.where(y != np.sign(X @ w))[0]
            score = idx_incorrect.size
            if score == 0:
                best_w = w
                break
            idx = np.random.choice(idx_incorrect, 1)
            w += y[idx] * X[idx, :].flatten()
            if score < best_score:
                best_w = w
                best_score = score
    ...
    def score(self, X, y):
        ...
        score = np.sum(y == np.sign(X @ self.w))
        ### END CODE
        return score
...
def poly_transform(X):
    ...
    Xt = X
    ### YOUR CODE HERE
    Xt = np.column_stack([X[:, 0] ** i * X[:, 1] ** j for i in range(4) for j in range(4) if i + j <= 3])
    ### END CODE
    return Xt
