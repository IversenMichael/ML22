...
def test_housing(max_depth, random_state=42):
    ...
    for d in [1, 7, 20]:
        ...
        print('Normal Regression Tree - Trainining data Least Squares Loss',
              ((tree.predict(X_train) - y_train) ** 2).mean())
        print('Normal Regression Tree Normal - Test data Least Squares Loss',
    ...
    ax.set_title('Loss Per Iteration for L2 Stagewise Additive Modelling - Max Depth {0}'.format(max_depth),
                 fontsize=20)
    plt.show()
...
if __name__ == '__main__':
    max_depth = 1
    random_state = 42
    test_housing(max_depth, random_state)