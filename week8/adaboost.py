...
class AdaBoostClassifier:
    ...
    def fit(self, X, y):
        ...
        for t in range(self.n_estimators):
            ...
            idx_incorrect = np.where(pred != y)[0]
            err = np.sum(D[idx_incorrect])
            self.alphas.append(0.5 * np.log((1 - err) / err))
            Z = np.sum(D * np.exp(-self.alphas[t] * y * pred))
            D = D * np.exp(-self.alphas[t] * y * pred) / Z
            scores.append(self.score(X, y))
            exp_losses.append(np.mean(np.exp(-y * self.predict(X))))
        ### END CODE
    ...
    def exp_loss(self, X, y):
        ...
        loss = np.mean(np.exp(- y * self.predict(X)))
        ### END CODE
        return loss
